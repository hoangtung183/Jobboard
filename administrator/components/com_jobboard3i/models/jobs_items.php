...
<?php
	...
	class Jobboard3iModelJobs_items extends JModel {
		function getData(){
	        ...
	        $query = "SELECT * FROM #__jb_jobs_item";
	        $conditions = array();
	    ...
	    function getTotal()
	    {
	    	...
	    	$query = "SELECT COUNT(*) FROM #__jb_jobs_item WHERE id>0";
	    ...
	    function unpublish()
	    {
	    	...
	    	$cache->clean();
	    	$mainframe->redirect('index.php?option=com_pos&view=clients');
	    }
	    ...
	    function remove()
	    {
	    	...
	    	$db = JFactory::getDBO();
	    	$cid = JRequest::getVar('cid');
	    	$row = JTable::getInstance('jobs_item', 'Table');
	    	foreach ($cid as $id)
	    	{
	    		$row->load($id);
	    		$row->delete($id);
	    	...
	    	$cache = JFactory::getCache('com_pos');
	    	$cache->clean();
	    	$mainframe->redirect('index.php?option=com_jobboard3i&view=jobs_items', JText::_('K2_DELETE_COMPLETED'));
