...
<?php
	...
	class Jobboard3iModelCompanies extends JModel {
		function getData(){
	        ...
	        $query = "SELECT * FROM #__jb_companies";
	        $conditions = array();
	    ...
	    function getTotal()
	    {
	    	...
	    	$query = "SELECT COUNT(*) FROM #__jb_companies  WHERE id>0";
	    ...
	    function unpublish()
	    {
	    	...
	    	$mainframe = JFactory::getApplication();
	    	$cid = JRequest::getVar('cid');
	    	$row = JTable::getInstance('jb_company', 'Table');
	    	foreach ($cid as $id)
	    	{
	    		$row->load($id);
	    		$row->publish($id, 0);
	    	...
	    	$cache->clean();
	    	$mainframe->redirect('index.php?option=com_jobboard3i&view=companies');
	    }
	    ...
	    function remove()
	    {
	    	...
	    	$cache->clean();
	    	$mainframe->redirect('index.php?option=com_jobboard3i&view=companies', JText::_('K2_DELETE_COMPLETED'));
