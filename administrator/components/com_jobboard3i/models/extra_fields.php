...
<?php
	...
	class Jobboard3iModelExtra_fields extends JModel {
		function getData(){
	        ...
	        $query = "SELECT * FROM #__jb_extra_field";
	        $conditions = array();
	    ...
	    function getTotal()
	    {
	    	...
	    	$query = "SELECT COUNT(*) FROM #__jb_extra_field  WHERE id>0";
	    ...
	    function remove()
	    {
	    	...
	    	$db = JFactory::getDBO();
	    	$cid = JRequest::getVar('cid');
	    	$row = JTable::getInstance('jb_extrafield', 'Table');
	    	foreach ($cid as $id)
	    	{
	    		$row->load($id);
	    		$row->delete($id);
	    	...
	    	$cache->clean();
	    	$mainframe->redirect('index.php?option=com_jobboard3i&view=extra_fields', JText::_('K2_DELETE_COMPLETED'));
