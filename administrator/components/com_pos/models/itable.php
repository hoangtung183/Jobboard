...
class PosModelitable extends JModel {

	/*public function getForm($data=array(),$loadData = true){
		$form=$this->loadForm ("com_pos.itable","itables");
   ...
   function save()
    {
        ...
        {
            $mainframe->redirect('index.php?option=com_pos&view=itable', $row->getError(), 'error');
        }

        if (!$row->check())
        {
            $mainframe->redirect('index.php?option=com_pos&view=itable&cid='.$row->id, $row->getError(), 'error');
        }
        ...
        switch(JRequest::getCmd('task'))
        {
            case 'apply' :
                ...
                $link = 'index.php?option=com_pos&view=itable&cid='.$row->id;
                break;
            case 'save' :
            default :
                $msg = JText::_('K2_TAG_SAVED');
                $link = 'index.php?option=com_pos&view=itables';
                break;
        }
