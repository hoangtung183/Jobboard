...
class PosModelcategory extends JModelAdmin {

	public function getForm($data=array(),$loadData = true)
	{
		$form = $this->loadForm("com_pos.category","category");
   ...
   function save()
    {	
        ...
        {
            $mainframe->redirect('index.php?option=com_pos&view=category', $row->getError(), 'error');
        }

        if (!$row->check())
        {
            $mainframe->redirect('index.php?option=com_pos&view=category&cid='.$row->category_id, $row->getError(), 'error');
        }

        if (!$row->store())
        {
            $mainframe->redirect('index.php?option=com_pos&view=categoryxx', $row->getError(), 'error');
        }
       ...
       if (!$row1->bind(JRequest::get('post')))
        {
            $mainframe->redirect('index.php?option=com_pos&view=category', $row1->getError(), 'error');
        }

        if (!$row1->check())
        {
            $mainframe->redirect('index.php?option=com_pos&view=category&cid='.$row1->id, $row1->getError(), 'error');
        }
        if (!$row1->store())
        {
         ...
         $query1 = "SELECT *   FROM category WHERE category_id=(SELECT MAX(category_id) FROM category)";
         $query2= "SELECT * FROM media WHERE media_id=(SELECT MAX(media_id) FROM media) ";
       //  $db1->setQuery($query1);
      //   $db2->setQuery($query2);
       //  $rows_category = $db1->loadObject();
      //   $rows_media=$db2->loadObject();
        	
       
    	$row2 = JTable::getInstance('media_category', 'Table'); 
    	
    	$category_id=$rows_category->category_id;
    	$media_id=$rows_media->media_id;
    	
    	
		/*$from = array( 'category_id'        => 6,
               'category'  => 'abc',
               'media_id'  => 2,
               'description' => 'hhh'
             );*/
    	...
    	//$id = $this->getLastId();
    	 $db = JFactory::getDBO();
    //	$mssql_query = "INSERT INTO category(name,description,create_time,update_time) VALUES('ca','ca','2013-04-04','2013-04-04')" ;	
  ...
  //  	$result = mssql_fetch_assoc(mssql_query("select @@IDENTITY as id"));
	...
	/*
       ...
       if (!$row2->bind($array))
        {
            $mainframe->redirect('index.php?option=com_pos&view=category', $row2->getError(), 'error');
        }

        if (!$row2->check())
        {
            $mainframe->redirect('index.php?option=com_pos&view=category&cid='.$row2->id, $row2->getError(), 'error');
        }
        if (!$row2->store())
        {
        ...
        switch(JRequest::getCmd('task'))
        {
            case 'apply' :
                ...
                $link = 'index.php?option=com_pos&view=category&cid='.$row->id;
                break;
            case 'save' :
            default :
